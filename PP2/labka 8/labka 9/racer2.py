...
COINS = 0
# background image
back = pygame.image.load("labka 9/images/street.png")
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self):
        ...
        super().__init__()
        image = pygame.image.load("labka 9/images/racer1.png")
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self):
        super().__init__()
        image = pygame.image.load("labka 9/images/Pygame_rects.png")
        self.image = pygame.transform.scale(image, (80, 177))
        self.rect = self.image.get_rect()
...
class Coins(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.chance = 0
        self.coins = [
            "labka 9/images/Cion.png",
            "labka 9/images/Star.png"
        ]
...
pygame.mixer.music.load("labka 9/audio/Better Day.mp3")
...
while True:
    ...
    coins = font_small.render(str(COINS), True, (150,210,140))
    screen.blit(coins, (WIDTH - 30, 10))
    ...
    if pygame.sprite.spritecollideany(P1, game_coins):
        pygame.mixer.Sound('labka 9/audio/Coin Touch.wav').play()
        if C1.chance == 0:
            COINS += 1
        else:
            COINS += 5
        C1.change_coin()
        C1.rect.top = 0
        C1.rect.center = (random.randint(50, WIDTH - 50), 0)
        # random_timing = random.randint(0, 10000)

    # To be run if collision occurs between Player and Enemy
    if pygame.sprite.spritecollideany(P1, enemies):
        pygame.mixer.music.stop()
        pygame.mixer.Sound('labka 9/audio/Accident Sound.wav').play()
        time.sleep(0.5)

        # showing texts on endgame screen
        collected_coins = font_small.render(f"Collected coins: {COINS}", True, (0,0,0))
        final_score = font_small.render(f"Score: {SCORE}", True, (0,0,0))
        screen.fill((255,0,0))
        screen.blit(game_over, (WIDTH / 2 - 130, HEIGHT / 2 - 30))
        screen.blit(final_score, (WIDTH / 2 - 130, HEIGHT / 2 + 30))
        screen.blit(collected_coins, (WIDTH / 2 - 130, HEIGHT / 2 + 50))
